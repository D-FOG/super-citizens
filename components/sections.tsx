...
export function Hero() {
  return (
    <section className="relative isolate min-h-screen overflow-hidden pt-28">
      <div className="absolute inset-0 bg-hero-grid bg-[length:72px_72px] opacity-40" />
      <div className="section-shell relative grid min-h-[calc(100vh-7rem)] items-center gap-12 pb-16 lg:grid-cols-[1.05fr_.95fr]">
        <Reveal>
          <div>
            <p className="mb-5 inline-flex rounded-full border border-accent/35 bg-accent/10 px-4 py-2 text-xs font-black uppercase tracking-[.18em] text-accent">
              Your Result Shows It.
            </p>
            <h1 className="max-w-5xl text-balance text-5xl font-black leading-[.92] sm:text-6xl lg:text-7xl">
        ...
        <Reveal delay={0.12}>
          <div className="cinematic-panel relative overflow-hidden rounded-lg p-3">
            <img src={imageSet.worship} alt="People gathered in worship" className="image-treatment h-[28rem] w-full rounded-md object-cover" />
            <div className="absolute inset-x-6 bottom-6 grid gap-3 rounded-md border border-white/15 bg-black/60 p-4 text-white backdrop-blur-md sm:grid-cols-3">
              {[
                ["12+", "Growth Tracks"],
                ["7", "Cluster Levels"],
                ...
                <div key={label}>
                  ...
                  <span className="text-xs font-semibold uppercase tracking-[.14em] text-white/70">{label}</span>
                </div>
              ))}
            </div>
          </div>
...
export function SectionHeader({ eyebrow, title, text }: { eyebrow: string; title: string; text?: string }) {
  return (
    <Reveal className="mx-auto mb-10 max-w-3xl text-center">
      <p className="mb-3 text-xs font-black uppercase tracking-[.22em] text-accent">{eyebrow}</p>
      <h2 className="text-balance text-3xl font-black sm:text-5xl">{title}</h2>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {systemPillars.map((pillar, index) => (
            <Reveal key={pillar.title} delay={index * 0.05} className="premium-card p-6">
              <pillar.icon className="mb-8 h-8 w-8 text-accent" />
    ...
    <section className="py-24">
      <div className="section-shell grid gap-5 lg:grid-cols-2">
        <Reveal className="premium-card p-7">
          <p className="text-xs font-black uppercase tracking-[.2em] text-muted">The problem</p>
          <h2 className="mt-4 text-4xl font-black">Activity without structure cannot carry destiny.</h2>
          <div className="mt-8 grid gap-3">
            {problems.map((problem) => (
              <div key={problem} className="flex items-center gap-3 rounded-md border border-line/70 p-4">
        ...
        <Reveal delay={0.08} className="cinematic-panel overflow-hidden rounded-lg">
          <img src={imageSet.leaders} alt="Leaders planning a structured system" className="image-treatment h-64 w-full object-cover" />
          <div className="p-7">
            <p className="text-xs font-black uppercase tracking-[.2em] text-accent">The solution</p>
            <h2 className="mt-4 text-4xl font-black">Intentional growth, practical training, real-world deployment.</h2>
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          {workflow.map((step, index) => (
            <Reveal key={step} delay={index * 0.05} className="relative premium-card p-6">
              <span className="mb-8 grid h-12 w-12 place-items-center rounded-md bg-accent text-lg font-black text-black">{index + 1}</span>
          ...
          {activities.map((activity, index) => (
            <Reveal key={activity.title} delay={index * 0.05} className="premium-card p-6">
              ...
              <div className="mt-10 flex items-end justify-between gap-3">
                ...
                <div className="rounded-md border border-line/70 p-3 text-right">
...
export function FinalCta() {
  return (
    <section className="py-24">
      <div className="section-shell">
        <Reveal className="cinematic-panel overflow-hidden rounded-lg p-8 text-center sm:p-14">
          <h2 className="mx-auto max-w-4xl text-balance text-4xl font-black sm:text-6xl">
  ...
  return (
    <section className="pt-32">
      <div className="section-shell grid gap-8 lg:grid-cols-[.95fr_1.05fr]">
        <Reveal>
          <p className="text-xs font-black uppercase tracking-[.22em] text-accent">{eyebrow}</p>
          <h1 className="mt-4 text-balance text-5xl font-black sm:text-6xl">{title}</h1>
          <p className="mt-6 text-lg leading-8 text-muted">{text}</p>
        </Reveal>
        <Reveal delay={0.08} className="cinematic-panel overflow-hidden rounded-lg p-3">
          <img src={image} alt="" className="image-treatment h-[28rem] w-full rounded-md object-cover" />
        </Reveal>
      </div>
      <div className="section-shell grid gap-4 py-20 md:grid-cols-2">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
          {clusterLevels.map((level, index) => (
            <Reveal key={level} delay={index * 0.04} className="premium-card p-5">
              <span className="text-xs font-black uppercase text-accent">Level {index + 1}</span>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          {trainings.map((training, index) => (
            <Reveal key={training.title} delay={index * 0.05} className="premium-card p-6">
              <training.icon className="h-7 w-7 text-accent" />
              <span className="mt-8 inline-block rounded-full bg-accent/10 px-3 py-1 text-xs font-black text-accent">{training.tag}</span>
              <h3 className="mt-4 text-xl font-black">{training.title}</h3>
              <p className="mt-3 text-sm leading-6 text-muted">{training.text}</p>
              <div className="mt-6 h-2 overflow-hidden rounded-full bg-line/60">
                <div className="h-full rounded-full bg-accent" style={{ width: `${44 + index * 11}%` }} />
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {books.map((book, index) => (
            <Reveal key={book} delay={index * 0.05} className="premium-card overflow-hidden">
              <div className="grid aspect-[4/5] place-items-end bg-gradient-to-br from-accent/20 via-royal/20 to-gold/20 p-5">
              ...
              <div className="p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {accessLevels.map((level) => (
                    <span key={level} className="rounded-full border border-line/70 px-3 py-1 text-xs font-bold text-muted">{level}</span>
                  ))}
                </div>
              </div>
            </Reveal>
...
function BookCover({ title }: { title: string }) {
  return (
    <div className="w-full rounded-md border border-white/20 bg-black/75 p-5 text-white shadow-soft">
      <LockKeyhole className="mb-12 h-6 w-6 text-accent" />
      <p className="text-lg font-black leading-tight">{title}</p>
      <p className="mt-4 text-xs uppercase tracking-[.18em] text-white/60">Supersite Citizens</p>
    </div>
  );
}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {events.map((event, index) => (
            <Reveal key={event.title} delay={index * 0.05} className="premium-card p-6">
              ...
              <p className="mt-8 text-xs font-black uppercase tracking-[.18em] text-muted">{event.type}</p>
...
export function InlineNextAction({ href, label }: { href: string; label: string }) {
  return (
    <div className="section-shell pb-20">
      <Reveal className="flex flex-col items-start justify-between gap-4 rounded-lg border border-line/70 bg-accent/10 p-6 sm:flex-row sm:items-center">
        <div>
          <p className="text-xs font-black uppercase tracking-[.18em] text-accent">Next step</p>
